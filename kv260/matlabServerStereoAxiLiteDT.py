...
if simulink == True:
    ...
    simulinkMem.write(struct.pack('l', 100))       # threshold value
    simulinkMem.seek(4) 
...
while(1):
    ...
    #print(cmd)
    if cmd == '0':
        threshold = int(npSocket.receiveParam())
        print(threshold)
        simulinkMem.seek(256) 
        simulinkMem.write(struct.pack('l', threshold))      # reset IP core
        ...
        toc = time.time() 
        delta = toc - tic
        tempImageLeft = cv2.putText(tempImageLeft, str(delta), (50,50), cv2.FONT_HERSHEY_SIMPLEX ,  
                   1, 0, 1, cv2.LINE_AA) 
